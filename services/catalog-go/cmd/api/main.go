...
func main() {
	...
	r.POST("/admin/login", func(c *gin.Context) {
		...
		// If this is a fresh install (no users yet), persist the bootstrap user into DB.
		// This makes admin login consistent even if env vars later change.
		var existingID int64
		if err := db.QueryRow("SELECT id FROM admin_users WHERE username = ? LIMIT 1", body.Username).Scan(&existingID); err != nil {
			if hash, err2 := bcrypt.GenerateFromPassword([]byte(body.Password), bcrypt.DefaultCost); err2 == nil {
				_, _ = db.Exec(
	...
	admin.POST("/products", func(c *gin.Context) {
		...
		_ = cache.Del(context.Background(), "products:list").Err()
		c.JSON(http.StatusCreated, gin.H{"id": id})
	})
